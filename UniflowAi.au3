...
Global Const $Theme_dir = @ScriptDir & "\GUI\Themes"
...
If FileExists(@ScriptDir & "\Themes") = 0 Then DirCreate(@ScriptDir & "\Themes")
...
; Title Label
GUICtrlCreateLabel('XeniaFlow', 0, 0, 1280, 40, BitOR($SS_CENTER, $BS_CENTER), $GUI_WS_EX_PARENTDRAG)
GUICtrlSetFont(-1, 20, 800)
...
; Uncomment and customize these lines to add ambient lights
...
Global $parentgui = GUICreate("", 1280, 720, 0, 40, $WS_POPUP, BitOR($WS_EX_LAYERED, $WS_EX_MDICHILD), $Main_Flow_gui)
Global $Alphabet_gameName = GUICtrlCreateLabel('', 20, 0, 100, 50, $SS_CENTER)
GUICtrlSetFont(-1, 30, 600)
GUICtrlSetColor(-1, $AlphabColor)

Global $label_gameName = GUICtrlCreateLabel('', 1280/2-400, 30, 800, 50, $SS_CENTER)
GUICtrlSetFont(-1, 20, 400)
GUICtrlSetColor(-1, $TitleColor)

Global $next = GUICtrlCreateLabel(">>", 1220, 720/2-32, 64, 64)
GUICtrlSetFont(-1, 30, 400)
GUICtrlSetColor(-1, 0xFFFFFF)

Global $perv = GUICtrlCreateLabel("<<", 20, 720/2-32, 64, 64)
GUICtrlSetFont(-1, 30, 400)
GUICtrlSetColor(-1, 0xFFFFFF)
				...
				Case $msg = $Settings
					...
					If $ModeFlow = 1 Then
						If $stateSettings = 0 Then
						   AddCameraSceneNode(0, 0, 0, 160000, 0, 0, 0)
						...
						Else
						   ...
						   AddCameraSceneNode(0, 0, 0, 200, 0, 0, 0)
						   GUISetState(@SW_SHOW,  $parentgui)
						   $stateSettings = 0
		...
		If _IsPressed("01") Or _IsPressed("0D") Then
			...
			For $i = 0 To $Gamer_count Step 1
			   If $coll = $Cover[$i][0] And $coll = $Cover[$Centred_Game][0] And $stateSettings = 0 Then
				  ...
				  Local $Game_Path=IniRead(@ScriptDir & "\Config.ini","Xenia","Xbox360 Game Path","")
				  ...
				  If $extension2 = "mp4" Or $extension3 = "avi" Or $extension4 = "mkv" And $play = 0 Then; And $name = "default" Then
				  ...
				  If $extension1 = "xex" And $play = 0 Then; And $name = "default" Then
...
Func Draw($Start_point)
    ...
    For $index = 0 To $Gamer_count
        $Cover_Position[$index] = $Start_point + ($index * - $Cover_Distance)

        If $Cover_Position[$index] > -90 And $Cover_Position[$index] < 90 Then
            $rotationAngle = -$Cover_Position[$index]
        ElseIf $Cover_Position[$index] > 0 And $Cover_Position[$index] < 540 Then
            $rotationAngle = -90
        ElseIf $Cover_Position[$index] < 0 And $Cover_Position[$index] > -540 Then
            $rotationAngle = 90
        Else
            SetVisible($Cover[$index][0], 0)
            ContinueLoop
        EndIf

        SetRotation($Cover[$index][0], 0, $rotationAngle, 0)
        $CoverRunAnimator = CreateFlyCircleAnimator($Cover_Position[$index], 0, _GetAnimatorParameter($rotationAngle), 0, 0.0003, 0, 1, 0)
        AddAnimator($Cover[$index][0], $CoverRunAnimator)
        SetVisible($Cover[$index][0], 1)
    Next
...
Func _GetAnimatorParameter($rotationAngle)
    ; You might need to adjust this function based on your specific requirements
    Return (90 - Abs($rotationAngle)) / 1.5
EndFunc
	...
	If $Fileformat <> "" Then
		...
		For $j = 1 To $formatcount[0]
			If IniRead($INI, "BT", $formatcount[$j], -1) = 1 Then
			   ...
			   Local $Games = IniRead(@ScriptDir & "\Config.ini","Xenia","Xbox360 Game Path","")
